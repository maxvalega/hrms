...
@php
    ...
    $isJeminiDashboard = \App\Support\TenantHost::isJeminiMainPortal();
    $showClockCard = false;
@endphp
...
@push('css-page')
    <style>
        ...
        #mark-attendance { scroll-margin-top: 96px; }
...
@section('content')
    @php
        ...
        $isSpectalDashboard = \App\Support\TenantHost::subdomainFromHost() === 'spectal';
        $isJeminiDashboard = \App\Support\TenantHost::isJeminiMainPortal();
        if ($isJeminiDashboard && !isset($officeTime)) {
            $officeTime = [
                'startTime' => \App\Models\Utility::getValByName('company_start_time') ?: '09:00',
                'endTime' => \App\Models\Utility::getValByName('company_end_time') ?: '18:00',
            ];
        }
    ...
    <div class="dash-hero">
        <div class="dash-hero-inner">
            ...
            <div class="dash-hero-right">
                ...
                @if($isJeminiDashboard && $showClockCard)
                    <a href="{{ route('dashboard') }}#mark-attendance" class="hero-quick-btn is-primary">
                        <i class="ti ti-clock"></i> {{ __('Clock In / Out') }}
                    </a>
                @endif
    ...
    <div class="row">
        ...
        {{-- On jemini, Clock In / Out sits at the top so it is not buried under the calendar --}}
        @if($showClockCard && $isJeminiDashboard)
            @include('dashboard.partials.mark_attendance_card')
        @endif
        ...
        {{-- Mark Attendance card (hidden on Spectal; on jemini it is already shown above the calendar) --}}
        @if($showClockCard && !$isJeminiDashboard)
            @include('dashboard.partials.mark_attendance_card')
        @endif
        @if (\Auth::user()->type == 'employee')
